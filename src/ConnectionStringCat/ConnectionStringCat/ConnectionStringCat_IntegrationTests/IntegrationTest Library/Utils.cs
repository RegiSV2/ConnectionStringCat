	...
	public class TestUtils
	{
		...
		public static void WriteEmbeddedResourceToFile(Assembly assembly, string embeddedResourceName, string fileName)
		{
			...
			if (fileContents == null)
				throw new ApplicationException("Failed to get embedded resource '" + embeddedResourceName + "' from assembly '" +
				                               assembly.FullName);
		...
		public static void WriteEmbeddedResourceToBinaryFile(Assembly assembly, string embeddedResourceName, string fileName)
		{
			...
			if (stream == null)
				throw new InvalidOperationException("Failed to get embedded resource '" + embeddedResourceName + "' from assembly '" +
				                                    assembly.FullName);
		...
		public void SaveDocument(string documentMoniker)
		{
			...
			var runningDocumentTableService =
				(IVsRunningDocumentTable) VsIdeTestHostContext.ServiceProvider.GetService(typeof (IVsRunningDocumentTable));
			uint docCookie;
			IntPtr docData;
